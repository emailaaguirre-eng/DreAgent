...
class LeaWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.tts_voice_id = ("en", "com")  # Default to English (US) - tuple of (lang, tld)
        ...
        try:
            ...
            if self.voice_only_mode and kind != "system":
                if kind == "user":
                    ...
                    # Show brief indicator for Lea's response
                    indicator = "🔊 Lea is responding..."
                    safe = html.escape(indicator)
    ...
    def on_stream_chunk(self, chunk: str):
        """Handle streaming response chunks - fixed to prevent word-per-line display issue"""
        ...
        if self.voice_only_mode:
            # Just accumulate for history, but don't display
        ...
        try:
            ...
            # Convert newlines to HTML breaks, but keep everything else intact
            ...
            if not self.streaming_message_started:
                ...
                # Store the cursor position at the end before adding new content
                cursor = self.chat_display.textCursor()
                cursor.movePosition(QTextCursor.MoveOperation.End)
                self.chat_display.setTextCursor(cursor)
                
                # Append new message block using insertHtml
                new_lea_block = f'<div style="margin: 6px 0;"><span style="color:{self.ASSIST_COLOR}; font-weight:600;">Lea:</span> <span id="streaming-response" style="color:{self.ASSIST_COLOR};">{safe_text}</span></div>'
                cursor.insertHtml(new_lea_block)
                
                # Store where we started for updates
                self.streaming_start_position = self.chat_display.textCursor().position()
            else:
                # Update the existing streaming message by replacing its content
                # Get current HTML
                html_content = self.chat_display.toHtml()
                lea_pattern = f'<span style="color:{self.ASSIST_COLOR}; font-weight:600;">Lea:</span>'
                
                # Find and replace the last Lea message
                if lea_pattern in html_content:
                    # Split at the last occurrence of "Lea:"
                    parts = html_content.rsplit(lea_pattern, 1)
                    
                    if len(parts) == 2:
                        # Find where the current message content ends
                        after_lea = parts[1]
                        
                        # Look for the closing </span></div> that ends this message
                        # We need to find the </span> that closes the content span, then </div> that closes the message
                        span_end = after_lea.find('</span>')
                        if span_end > 0:
                            # Get everything after the closing tags
                            div_end = after_lea.find('</div>', span_end)
                            if div_end > 0:
                                remaining = after_lea[div_end + 6:]  # +6 for "</div>"
                                
                                # Reconstruct: [before] + "Lea:" + [new content] + [after]
                                new_html = parts[0] + lea_pattern + f' <span style="color:{self.ASSIST_COLOR};">{safe_text}</span></div>' + remaining
                                
                                # Save scroll position
                                scrollbar = self.chat_display.verticalScrollBar()
                                scroll_value = scrollbar.value()
                                scroll_max = scrollbar.maximum()
                                at_bottom = (scroll_value >= scroll_max - 10)
                                
                                # Update the HTML
                                self.chat_display.setHtml(new_html)
                                
                                # Restore scroll to bottom if we were there
                                if at_bottom:
                                    scrollbar.setValue(scrollbar.maximum())
                                else:
                                    scrollbar.setValue(scroll_value)
            
            # Ensure we're scrolled to bottom for new content
            try:
                scrollbar = self.chat_display.verticalScrollBar()
                if scrollbar.value() >= scrollbar.maximum() - 50:  # If near bottom
                    scrollbar.setValue(scrollbar.maximum())
            except:
                pass
            ...
            # Fallback: just append the chunk if streaming hasn't started
            try:
                if not self.streaming_message_started:
                    safe_text = html.escape(self.current_streaming_response).replace("\n", "<br>")
        ...
        try:
            ...
            if self.tts_enabled and answer:
                try:
                    ...
                    if text_to_speak:
                        # Update status for voice-only mode
                        if self.voice_only_mode:
                        ...
                        def speak_text_gtts():
                            try:
                                # Get language and accent from settings
                                if self.tts_voice_id and isinstance(self.tts_voice_id, tuple):
                                    lang, tld = self.tts_voice_id
                                else:
                                    lang, tld = "en", "com"  # Default to English (US)
                                
                                # Create gTTS with selected language and accent
                                tts = gTTS(text=text_to_speak, lang=lang, tld=tld)
    ...
    def show_settings(self):
        ...
        if not TTS_AVAILABLE:
            tts_warning = QLabel("⚠️ TTS not available. Install with: pip install gtts")
        ...
        # Voice selection - gTTS supports different accents via TLD
        voice_label = QLabel("Select Voice/Accent:")
        ...
        if result == QDialog.DialogCode.Accepted:
            ...
            # Save voice selection (lang, tld tuple)
