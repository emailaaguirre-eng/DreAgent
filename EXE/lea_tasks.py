    ...
    def execute(self, **kwargs) -> TaskResult:
        try:
            ...
            for email in emails:
                ...
                # Create synopsis (2-3 sentences from body)
                if full_body:
                    ...
                    # Take first 200 characters and create synopsis
                    if len(text_body) > 200:
                        sentences = text_body[:500].split('.')
                        if len(sentences) >= 2:
                            processed['Synopsis'] = '. '.join(sentences[:3]).strip() + '.'
                        else:
                            processed['Synopsis'] = text_body[:200].strip() + '...'
                    else:
                        processed['Synopsis'] = text_body.strip()
...
class OutlookCalendarCheckTask(BaseTask):
    ...
    def execute(self, **kwargs) -> TaskResult:
        try:
            ...
            events = client.get_calendar_events(days_ahead=30)
