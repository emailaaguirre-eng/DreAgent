...
| R2 | Outlook-only coupling | High | Medium | **In progress / partially mitigated (M2+M3 foundation)** | Product and architecture stuck on one mail/calendar stack; Gmail and multi-provider blocked | Provider port + Outlook adapter + registered Gmail skeleton. Remaining: live Gmail OAuth/token/API read, Outlook-only export/write routes, multi-provider UI |
...
| 2026-08-11 | R2 updated for M3 Gmail foundation skeleton (`feature/lea-gmail-read`); live Gmail read still deferred |
